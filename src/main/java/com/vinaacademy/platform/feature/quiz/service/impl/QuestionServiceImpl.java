...
public class QuestionServiceImpl implements QuestionService {
  ...
  public QuestionDto createQuestion(UUID quizId, QuestionDto questionDto) {
    ...
    Question question =
        Question.builder()
            ...
            .questionType(
                questionDto.getQuestionType() != null
    ...
    if (questionDto.getAnswers() != null && !questionDto.getAnswers().isEmpty()) {
      questionDto
          .getAnswers()
          .forEach(answerDto -> answerService.createAnswer(savedQuestion.getId(), answerDto));
    }
  ...
  private void updateQuestionType(Question question, QuestionType questionType) {
    ...
    if (questionType == QuestionType.TRUE_FALSE) {
      Answer trueAnswer = Answer.builder().answerText("Đúng").isCorrect(true).build();

      Answer falseAnswer = Answer.builder().answerText("Sai").isCorrect(false).build();
  ...
  private void updateQuizTotalPoints(Quiz quiz) {
    ...
    double totalPoints = questions.stream().mapToDouble(Question::getPoint).sum();
