...
public class QuizInstructorServiceImpl implements QuizInstructorService {
  ...
  public List<QuizDto> getQuizzesByCourseId(UUID courseId) {
    ...
    return quizzes.stream().map(QuizMapper.INSTANCE::quizToQuizDto).collect(Collectors.toList());
  }
  ...
  public List<QuizSubmissionResultDto> getQuizSubmissions(UUID quizId) {
    List<QuizSubmission> submissions =
        quizSubmissionRepository.findByQuizIdAndUserIdOrderByCreatedDateDesc(quizId, null);
  ...
  @Override
  public List<QuizStudentAttemptsDto> getStudentAttempts(UUID quizId) {
    List<QuizSubmission> submissions =
        quizSubmissionRepository.findByQuizIdOrderByCreatedDateDesc(quizId);

    var grouped =
        submissions.stream().collect(Collectors.groupingBy(s -> s.getQuizSession().getUser()));

    return grouped.entrySet().stream()
        .map(
            e -> {
              var dto = new QuizStudentAttemptsDto();
              dto.setStudent(UserMapper.INSTANCE.toDto(e.getKey()));
              dto.setAttempts(
                  e.getValue().stream()
                      .map(QuizSubmissionMapper.INSTANCE::toSubmissionResultDto)
                      .toList());
