...
public class AuthenticationServiceImpl implements AuthenticationService {
   ...
   *
   * <p>Creates a disabled user with an auto-generated username and the STUDENT role, persists it,
   ...
   * @throws BadRequestException if the provided passwords do not match or if the email is already
   *     registered
   */
  @Transactional
   ...
   *
   * <p>The returned username is the lowercase full name with all whitespace removed, truncated to
   * at most 10 characters, then appended with 5 random characters (resulting in up to 15
   * characters). If the generated username already exists in the user repository, a
   * RetryableException is thrown to allow the caller's retry mechanism to attempt a new generation.
   *
  ...
  @Retryable(
      ...
      maxAttempts = RetryConstants.DEFAULT_MAX_ATTEMPTS)
  ...
  public AuthenticationResponse login(AuthenticationRequest loginRequest) {
    ...
    authenticateUser(user.getUsername(), loginRequest.getPassword());
  ...
  private void authenticateUser(String username, String password) {
    try {
      ...
      if (ex instanceof DisabledException) {
        handleDisabledException(username);
      }
  ...
  private void handleDisabledException(String email) {
    ...
    ActionToken token =
        actionTokenRepository
            .findForUpdate(user, ActionTokenType.VERIFY_ACCOUNT)
  ...
  public void resendNewVerificationEmail(String email) {
    ...
    ActionToken actionToken =
        actionTokenRepository
            .findByUserAndType(user, ActionTokenType.VERIFY_ACCOUNT)
            .orElseThrow(() -> BadRequestException.message("Không tìm thấy token xác thực"));

    actionToken.setExpiredAt(
        LocalDateTime.now().plusHours(AuthConstants.ACTION_TOKEN_EXPIRED_HOURS));
  ...
  public void logout(RefreshTokenRequest refreshToken) {
    ...
    RefreshToken token =
        refreshTokenRepository
            .findByTokenAndUsername(refreshToken.getRefreshToken(), username)
  ...
  public void verifyAccount(String token, String signature) {
    ...
    ActionToken actionToken =
        actionTokenRepository
            .findByTokenAndType(token, ActionTokenType.VERIFY_ACCOUNT)
            .orElseThrow(() -> BadRequestException.message("Token không hợp lệ"));
    ...
    logService.log(
        ...
        LogConstants.VERIFY_ACCOUNT_ACTION,
        null,
  ...
  public boolean checkResetPasswordToken(ResetPasswordRequest request) {
    ...
    ActionToken actionToken =
        actionTokenRepository
            .findByTokenAndType(request.getToken(), ActionTokenType.RESET_PASSWORD)
            .orElseThrow(() -> BadRequestException.message("Token không hợp lệ"));
   ...
   * verifies the new password matches its confirmation, updates the user's stored password, deletes
   * the used action token, and records the action in the audit log.
   *
   * @param request contains the reset token, URL signature, new password, and retyped password
   * @throws BadRequestException if the signature is invalid, the token is not found, the token is
   *     expired, or the provided passwords do not match
   */
  @Override
  public void resetPassword(ResetPasswordRequest request) {
    ...
    logService.log(
        LogConstants.AUTH_KEY,
        LogConstants.RESET_PASSWORD_ACTION,
        null,
        Map.of("email", user.getEmail()));
  }
   ...
   *
   * <p>Validates that the provided new password matches the re-typed confirmation and that the
   ...
   * @throws BadRequestException if the new password and confirmation do not match or if the current
  ...
  public boolean changePassword(ChangePasswordRequest request) {
    ...
    logService.log(
        LogConstants.USER_KEY,
        LogConstants.CHANGE_PASSWORD_ACTION,
        null,
